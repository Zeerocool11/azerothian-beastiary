...
local function GetNPCID(unit)
    ...
    -- Vehicles can also be encountered, so allow them too.
...
local function ScanVisibleNameplates()
    ...
    for i = 1, 40 do
        ...
        if UnitExists(unit) then
            ...
            for _ in pairs(AzerothianBeastiaryDB.npcs) do
...
local function ListNPCs()
    ...
    for npcID, npcData in pairs(AzerothianBeastiaryDB.npcs) do
...
local function ScanCurrentTarget()
    ...
    local isNew =
        not AzerothianBeastiaryDB.npcs[npcID]
...
local function ExportNPCList()
    ...
    print(
        "|cff00bfff========== ABC EXPORT ==========|r"
    )

    print(
        "|cffaaaaaaNPC ID\tName|r"
    )
